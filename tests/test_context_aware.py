...
def test_kg_fallback():
    ...
    _reset()
    conn = db.get_conn()
    conn.execute("INSERT INTO entities(name, aliases, canonical) VALUES(?,?,?)", ("林下有", "[]", "林下有"))
    conn.commit()
    # 命中：林下有是明确实体
    assert _resolve_task_context(None, "林下有 产地", conn) == "林下有"
    # 未命中：泛词不兜底
...
def test_explicit_overrides():
    ...
    _reset()
    conn = db.get_conn()
    assert _resolve_task_context("GEO名片", "达人筛选", conn) == "GEO名片"
    # 会话记忆已被显式值覆盖
    assert mcp_api._session_task_context == "GEO名片"
...
def test_clear_context():
    ...
    # 清除后 KG 兜底生效
    assert _resolve_task_context(None, "林下有 品牌", conn) == "林下有"
